    ...
    update(t, dt) {
		...
		this.platforms.children.iterate(child => {
			...
			if (platform.y >= scrollY + 700){
                // console.log(platform.y, scrollY);
        ...
        let platformArr = [];
        this.platforms.children.iterate(child => {
            platformArr.push(child);
        })
        let playerY = this.player.y;
        platformArr.sort((ele1, ele2) => ele2 - ele1).forEach(child => {
            const platform = child;

            if (platform.y >= playerY) return;
            if (platform.y < playerY - 225) {
                platform.y = playerY - 225;
                platform.body.updateFromGameObject();
                playerY = platform.y;
            }
        })
